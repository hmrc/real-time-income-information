...
class RealTimeIncomeInformationControllerSpec extends PlaySpec with MockitoSugar with ScalaFutures with WireMockHelper with BaseSpec with IntegrationPatience {
  ...
  override protected val portConfigKey: String = "microservice.services.des-hod.port"
  ...
  protected lazy val authConnector: RTIIAuthConnector = injector.instanceOf[RTIIAuthConnector]
  ...
  "RealTimeIncomeInformationController" should {
    "Return 200 provided a valid request" when {
      "the service returns a successfully filtered response" in  {
        ...
        val sut = createSUT(service, auditService, authConnector)
...
//    }
  ...
  def createSUT(rtiiService: RealTimeIncomeInformationService, auditService: AuditService, rtiiAuthConnector: RTIIAuthConnector) =
